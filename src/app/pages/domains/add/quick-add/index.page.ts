...
interface QuickDomainInfo {
  ...
  ssl?: {
    issuer?: string | null; valid_from?: string; valid_to?: string; subject?: string;
    key_size?: number; signature_algorithm?: string; issuer_country?: string; fingerprint?: string;
  };
  host?: {
    query?: string; country?: string; city?: string; region?: string; isp?: string;
    org?: string; lat?: number; lon?: number; timezone?: string; as?: string;
  };
}
...
export default class QuickAddDomain {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      const domainInfo = (await lastValueFrom(
        this.http.get<{ domainInfo: QuickDomainInfo }>(`${domainInfoEndpoint}?domain=${domainName}`)
      ))?.domainInfo;
  ...
  private searchForSubdomains(domainName: string) {
    ...
    this.http.get<{ subdomain: string }[]>(`${domainSubsEndpoint}?domain=${domainName}`).pipe(
   ...
   */
  private handleDiscoveredSubdomains(validSubdomains: { subdomain: string }[], domainName: string): Observable<unknown> {
