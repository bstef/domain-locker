...
export async function updateSSL(
  ...
  const fields = [
    ...
    { label: 'Valid From', column: 'valid_from', type: 'date', old: existing?.valid_from ?? '', new: toDateOnly(fresh['valid_from'] as string | null | undefined), changeType: 'ssl_valid_from' },
    { label: 'Valid To', column: 'valid_to', type: 'date', old: existing?.valid_to ?? '', new: toDateOnly(fresh['valid_to'] as string | null | undefined), changeType: 'ssl_valid_to' },
